...
def usage():
    ...
    parser.add_argument('--img_info_path', type=str, help="img path guide")
...
data_loader = get_data_loader_folder(opts.input_folder, batch_size=1, train=False, new_size=new_size, height=fake_height, width=fake_width, crop=False, num_workers=config['num_workers'])
...
with torch.no_grad():
    ...
    if opts.trainer == 'MUNIT':
        ...
        print('Start testing')
        for i, (images, path) in tqdm(enumerate(data_loader)):
            if opts.number != -1 and i >= opts.number:
                break
            basename = path[0].split('/')[-1]
           # images = images[0]
            images = Variable(images.cuda(), volatile=True)
            code, _ = encode(images)
            if opts.recon:
