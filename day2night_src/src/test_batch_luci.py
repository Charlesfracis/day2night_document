...
from utils import get_config, get_data_loader_folder, pytorch03_to_pytorch04, get_data_loader_folder_inceptio
from trainer import MUNIT_Trainer, UNIT_Trainer
import argparse
from subprocess import call
from torch.autograd import Variable
from data import ImageFolder, InceptioImageFolder
...
def usage():
    ...
    parser.add_argument('--img_info_path', type=str, help="img path guide", required=True)
...
data_loader = get_data_loader_folder_inceptio(opts.input_folder, opts.img_info_path, camera_type='obstacle', batch_size=1, train=False, new_size=new_size, height=fake_height, width=fake_width, crop=False, num_workers=config['num_workers'])
...
with torch.no_grad():
    ...
    if opts.trainer == 'MUNIT':
        ...
        for i, images in tqdm(enumerate(data_loader)):
            ...
            outputs = decode(code)
            # outputs = (outputs + 1) / 2.
            basename = data_loader.dataset.imgs[i]['file_name']
